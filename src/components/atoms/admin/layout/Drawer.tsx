      ...
      <div className="px-3">
        <div className="flex justify-between items-start py-4">
          <div className="flex items-center justify-center">
            <p className="text-2xl font-medium text-primaryColor text-white">
              Ravi R.
            </p>
        ...
        <div className="py-6 flex flex-col gap-3 items-center">
          {ADMIN_NAV_ITEMS.map((item: AdminNavItems, index: number) => (
            <Link
              href={item.link}
              key={index}
              className={`p-2 w-full flex gap-3 items-center justify-start rounded-md text-center text-lg font-normal text-white hover:text-white hover:bg-red-400 cursor-pointer transition-all duration-100 ${
