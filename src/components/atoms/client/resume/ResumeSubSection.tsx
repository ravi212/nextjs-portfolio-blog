...
const ResumeSubSection = ({selectedSection}: {selectedSection : ResumeSectionProps}) => {
  return (
    <div className="w-[100%] border-t border-t-primaryColor">
    {/* <h3>{selectedSection.title}</h3>
    <p>{selectedSection.description}</p> */}
    <div className="flex flex-col my-4">
      {selectedSection.handle == "experience" && (
        <div className="grid grid-col-1 xl:grid-cols-2 gap-6 my-3">
          {selectedSection.items.map((item: any, index: number) => (
            <div
              key={index}
              className="p-6 bg-primaryColor text-secondaryColor rounded-2xl"
            >
